...
这是 2026-09-19 在 DM10（10 自由度双足，UniLab + MuJoCo）上做的一轮测量。
目的是搞清楚「训练时硬件到底在干什么」，顺便找优化点。

测量环境：ROG G16 GU605MV / Intel Core Ultra 9 185H（16 物理核 22 逻辑核）/ RTX 4060 Laptop 8G。
任务：`dm10_joystick_flat`，PPO，512 envs，MuJoCo 物理跑 CPU。

---

## 先说结论

**大头只有一块：物理仿真，占一个控制步的 71.3%，已经跑到 17.96 核。**

剩下的是三块串行代码，加起来 28.7%，即使全消掉也就 1.4 倍。
物理那块受限于内存带宽，加核、提频都没用（下面有实测）。

所以能显著改变局面的只有一条：把物理从 CPU 搬走（mjwarp），预期 1.9 倍左右。
要么就接受现状。
